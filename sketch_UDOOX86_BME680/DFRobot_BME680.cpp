...
boolean DFRobot_BME680::begin(void)
{
...
{
  bme680_sensor.write(bme680_sensor.dev_id, 0x74, (uint8_t*)&convertCmd, 1);
}
...
static int8_t read_field_data(struct bme680_field_data *data, struct bme680_dev *dev)
{
	...
	do {
      ...
      data->gas = (float)((uint16_t) buff[13] << 2 | (uint16_t) buff[14] >> 6) / 1024 * 500;
			...
			if (data->status & BME680_NEW_DATA_MSK) {
				data->temperature = (float) calc_temperature(adc_temp, dev) / 100.0f;
